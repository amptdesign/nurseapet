			...
			// Body
				...
				_body.h5u_reposition = function() {
					if (isTouch && (window.orientation == 0 || window.orientation == 180))
						...
						var vSpacer = (((_window.height() - panels[firstPanelId].height()) / 2) - _nav.height());
						if (vSpacer <= 215) {vSpacer = 215;}
						//alert(vSpacer);
						_wrapper.css('padding-top', vSpacer + 'px');
				};
						...
						// Fade out active panel
							...
							panels[activePanelId].fadeOut(settings.fadeSpeed, function() {
								...
								// Set new active
									...
									// Resize main to height of new panel  // '+ 12' ADDS JUST A LITTLE PADDING ON BOTTOM
										_main.animate({
											height: panels[activePanelId].outerHeight() + 12
